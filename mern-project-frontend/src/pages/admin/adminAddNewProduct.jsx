...
export default function AddProductPage() {
	...
	const [price, setPrice] = useState(0);                   //we can set it as 0 because price can't be negative
	const [labelledPrice, setLabelledPrice] = useState(0);
	const [category, setCategory] = useState("cream");       //default category is cream
	const [stock, setStock] = useState(0);                   // if the stock is not given, it will be 0(default value)
    ...
    async function addProduct() {
        ...
        const promises = [];     // create promises to upload images
		for (let i = 0; i < images.length; i++) {
			promises[i] = mediaUpload(images[i]);
		}

		try{
			const urls = await Promise.all(promises) // get the urls of images 
			const alternativeNames = altNames.split(",") //when input alt names, they should split using commas
			...
			// call the backend using axios to add products
			axios.post(import.meta.env.VITE_API_URL+"/api/products" ,product,{      // (/api/products) this one called in the index.js and it will directly go to the product router and then go tothe create product
				headers: {

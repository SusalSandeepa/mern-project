...
export async function createOrder(req,res){
    ...
    try{
        ...
        if(!Array.isArray(itemsInRequest)){  //if itemsInRequest is not an array
            res.status(400).json(
                {
        ...
        for (let i=0; i<itemsInRequest.length; i++){
            ...
            const product = await Product.findOne({productID : item.productID}) // check if the products in the request and the database are the same one by one

            if(product == null) //if the product is not found in the database
            {
                res.status(400).json(
                    {
                        code: "not-found",
            ...
            if(product.stock < item.quantity){
                res.status(400).json({
                        code : "stock",
                        message: `Insufiicient stock for product with ID ${item.productID}`,
                        productID: item.productID,
            ...
            itemsToBeAdded.push({         //push the item to the array
        ...
        for(let i=0; i<itemsToBeAdded.length; i++){
            ...
            await Product.updateOne(
                {productID: item.productID},
                {$inc : {stock : -item.quantity}}  //decrement the stock
            )
        }
        */
...
export async function getOrders(req,res){
    if(isAdmin(req)){
        const orders = await Order.find().sort({date:-1}) // able to see all orders sorted by date
        res.json(orders)
    }else if(isCustomer(req)){
        const orders = ((await Order.find({email: user.email})).sort({date:-1})) // able to see all orders only of the user by email sorted by date
        res.json(orders)
